        ...
        <div class="notice-box-area-centerRight">
          ...
          <div>
            ...
            <div class="pa-detail-areaAll" :class="total <= 10?'pa-detail-areaAllClass' : ''">
              <div class="pa-detail-area" v-for="(item,index) in listData1" :key='index'><span><el-checkbox v-model="list[index]" @change="selectFunc(item)"></el-checkbox></span><span>{{item.account}}</span><span>{{item.nickName}}</span><span>{{item.phone}}</span><span>{{item.orgName}}</span></div>
...
<script>
  ...
  export default {
    name: 'selectUser',
    props: ['type', 'content', 'roleId', 'reload', 'orgName', 'orgId'],
    data () {
      return {
        departmentName: '',
        account: '',
    ...
    methods: {
      init () {
        ...
        ApiClient.get('/user/getRoleUnjoinedUser', {
          roleId: that.roleId,
          orgId: that.orgId,
          depName: that.departmentName.trim(),
      ...
      confirm () {
        this.$emit('confirm', this.rightData, this.roleId)
        this.rightData.splice(0, this.rightData.length)
        this.selectAllData = []
        this.checked = false
        this.departmentName = ''
    ...
    watch: {
      ...
      //       用户组id变化就加载
      //      'roleSelectId': function (val, oldval) {
      //        this.init()
      //      },
